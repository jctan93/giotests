...
def grab_dma_msg():
    ...
    list = splitter_comma.split(execute("dmesg -t | grep -i DMA | grep -i tty"))
    cleanup()
    if ("ttyS2 - failed to request DMA" in list and "ttyS3 - failed to request DMA" in list) and ("ttyS0 - failed to request DMA" not in list and "ttyS1 - failed to request DMA" not in list):
        return True   
    else:
        return False
