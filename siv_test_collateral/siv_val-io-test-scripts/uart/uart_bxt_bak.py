...
test_app = test_app_location + "ioh_uart"
...
ssh_dut = ssh_com(dev_1_ip)
ssh_thm = ssh_com(dev_2_ip)
ssh_thm.setpub(sshpass="ssvroot")
...
def loopback_tx_rx():
	...
	ssh_dut.ssh_exec("pkill -x ioh_uart")
	ssh_thm.ssh_exec("pkill -x ioh_uart")

	# Remove old UART log files from previous run
	# dut.timeexecute("rm -f /UART_LOG2; rm -f /UART_LOG")
	ssh_dut.ssh_exec("rm -f /UART_LOG2; rm -f /UART_LOG")
	...
	if mode == "rxr":
		...
		# dut.timeexecute(test_app + " " + uart_port_1 + " -"+ str(mode) + " " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG" )
		ssh_dut.ssh_exec(test_app + " " + uart_port_1 + " -"+ str(mode) + " " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG" )
		time.sleep(5)
		# dut.timeexecute(test_app + " " + uart_port_1 + " -txr " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG" )
		ssh_dut.ssh_exec(test_app + " " + uart_port_1 + " -txr " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG" )

	else:
		# dut.timeexecute(test_app + " " + uart_port_1 + " -"+ str(mode) + " " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG2" )
		ssh_dut.ssh_exec(test_app + " " + uart_port_1 + " -"+ str(mode) + " " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG2" )

	print test_app + " " + uart_port_1 + " -"+ str(mode) + " " + str(baud_rate) + " " + str(data_bit) + " " + parity + " " + str(stop_bits) + " " + flow_control + " " + str(iterations) + " n s n >> /UART_LOG2"
	...
	if mode =="rxr":
		result_2 = ssh_dut.ssh_exec("cat /UART_LOG")
	else:
		result_2 = ssh_dut.ssh_exec("cat /UART_LOG2")
	...
	if mode == "rxr":
		...
		if re.search('SUCCESS: Data sent and received are same', str(ssh_dut.ssh_exec("cat /UART_LOG2"))):
			print "VERDICT: PASS"
			return True
		else:
			print "VERDICT: FAIL"
...
def uart_manual_flow_control():
	...
	ssh_dut.ssh_exec("rm -rf /UART_FLOWCNTL_LOG")
	ssh_dut.ssh_exec(test_app_location + "uart_flowcntl >> /UART_FLOWCNTL_LOG")
	return ssh_dut.ssh_exec("cat /UART_FLOWCNTL_LOG")
...
def uart_line_break():
    ...
    # print print_log
	ssh_dut.ssh_exec("rm -rf /UART_LINE_BREAK_LOG")
	ssh_dut.ssh_exec(test_app_location + "siginit & >> /UART_LINE_BREAK_LOG")
	#time.sleep(5)
	ssh_dut.ssh_exec(test_app_location + "uart_line_break")
	print_log = ssh_dut.ssh_exec("cat /UART_LINE_BREAK_LOG")
	print print_log
