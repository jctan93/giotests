...
#
# To get the final UUID a.k.a disk identifier after partition and formatting a drive/partition
#
TARGET_DEVICE=$((fdisk -l $DEVICE | grep -i "disk identifier") 2>&1)
TARGET_DEVICE="${TARGET_DEVICE#*x}"
PARTITION_NUMBER="3"
TARGET_ROOTFS=$TARGET_DEVICE-02
echo $TARGET_ROOTFS
...
if [ -e "$GRUB_CFG" ]; then
	...
	sed -i "s@vmlinuz @vmlinuz root=PARTUUID=$TARGET_ROOTFS ro rootwait quiet @" $GRUB_CFG
fi
...
if [ -d "$GUMMI_ENTRIES" ]; then
	...
	sed -i "s@options *LABEL=boot @options LABEL=Boot root=PARTUUID=$TARGET_ROOTFS ro rootwait quiet @" $GUMMI_CFG
fi
